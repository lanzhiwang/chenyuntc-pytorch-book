...
def train(**kwargs):
    ...
    content_dataloader = data.DataLoader(
        content_dataset, opt.batch_size, shuffle=True, drop_last=True
    )
    style_dataset = tv.datasets.ImageFolder(opt.style_data_root, transfroms)
    style_dataloader = data.DataLoader(
        style_dataset, opt.batch_size, shuffle=True, drop_last=True
    )
    ...
    for epoch in range(opt.epoches):
        ...
        for ii, image in tqdm.tqdm(
            ...
            if (ii + 1) % opt.plot_every == 0:
                ...
                vis.img(
                    "input_content", (content.cpu()[0] * std + mean).clamp(min=0, max=1)
                )
                vis.img(
                    "input_style", (style.cpu()[0] * std + mean).clamp(min=0, max=1)
                )
                vis.img(
                    "output",
                    (model.generate(content, style)[0].cpu() * std + mean).clamp(
                        min=0, max=1
                    ),
                )
