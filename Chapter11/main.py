...
def gen_acrostic(**kwargs):
    ...
    for _ in range(max_len):
        ...
        if next_word in {word2ix["。"], word2ix["！"], word2ix["<START>"]}:
...
def train(**kwargs):
    ...
    dataloader = DataLoader(
        data, batch_size=opt.batch_size, shuffle=True, num_workers=1
    )
...
def generate_square_subsequent_mask(sz):
    mask = (t.triu(t.ones(sz, sz)) == 1).transpose(
        0, 1
    )  # 生成下三角矩阵（下三角全为True，其余位False）
    mask = (
        mask.float()
