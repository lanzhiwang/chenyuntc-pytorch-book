...
def train(**kwargs):
    ...
    dataset = COCODataset(
        data_dir=opt.data_path, img_size=512, transform=Augmentation()
    )
...
def test(**kwargs):
    ...
    for index, file in enumerate(os.listdir(opt.test_img_path)):
        ...
        x = t.from_numpy(
            transform(img, boxes=None, labels=None)[0][:, :, (2, 1, 0)]
        ).permute(2, 0, 1)
        x = x.unsqueeze(0).to(device)
        bbox_pred, score, cls_ind = model.generate(x)
        bbox_pred = bbox_pred * np.array(
        ...
        for i, box in enumerate(bbox_pred):
            if score[i] > 0.35:
                ...
                cv2.rectangle(
                    ...
                    (int(xmin), int(ymin)),
                    (int(xmax), int(ymax)),
                    class_color[int(cls_indx)],
                    2,
                )
                cv2.rectangle(
                    img,
                    (int(xmin), int(abs(ymin) - 15)),
                    (int(xmin + int(xmax - xmin) * 0.55), int(ymin)),
                    class_color[int(cls_indx)],
                    -1,
                )
                cv2.putText(
