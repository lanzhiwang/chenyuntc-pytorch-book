    ...
    for batch_index in range(batch_size):
        for gt_label in label_lists[batch_index]:
            ...
            if result:
                ...
                gt_tensor[
                    batch_index, grid_y, grid_x, num_classes : num_classes + 4
                ] = np.array([tx, ty, tw, th])
                gt_tensor[batch_index, grid_y, grid_x, num_classes + 4] = 1.0

                # 创建高斯热力图
                for i in range(
                    grid_x - 3 * int(sigma_w), grid_x + 3 * int(sigma_w) + 1
                ):
                    for j in range(
                        grid_y - 3 * int(sigma_h), grid_y + 3 * int(sigma_h) + 1
                    ):
                        if i < ws and j < hs:
                            v = np.exp(
                                -((i - grid_x) ** 2) / (2 * sigma_w**2)
                                - (j - grid_y) ** 2 / (2 * sigma_h**2)
                            )
...
class FocalLoss(nn.Module):
    ...
    def forward(self, inputs, targets):
        ...
        other_loss = (
            -other_id * (1 - targets) ** 4 * (inputs) ** 2 * t.log(1.0 - inputs + 1e-14)
        )
...
def get_loss(pred_cls, pred_txty, pred_twth, label, num_classes):
    ...
    txty_loss = (
        t.sum(
            t.sum(txty_loss_function(pred_txty, gt_txtytwth[:, :, :2]), 2)
            * gt_box_scale_weight
        )
        / batch_size
    )
    ...
    twth_loss = (
        t.sum(
            t.sum(twth_loss_function(pred_twth, gt_txtytwth[:, :, 2:]), 2)
            * gt_box_scale_weight
        )
        / batch_size
    )
