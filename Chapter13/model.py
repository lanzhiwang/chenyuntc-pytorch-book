...
class Model(nn.Module):
    ...
    def decode(self, pred):
        ...
        grid_y, grid_x = t.meshgrid(
            [t.arange(128, device=pred.device), t.arange(128, device=pred.device)]
        )
    ...
    def get_topk(self, scores):
        ...
        topk_inds = self.gather_feat(topk_inds.view(B, -1, 1), topk_ind).view(
            B, self.topk
        )
    ...
    def generate(self, x):
        ...
        txtytwth_pred = (
            t.cat([txty_pred, twth_pred], dim=1)
            .permute(0, 2, 3, 1)
            .contiguous()
            .view(B, -1, 4)
        )
        ...
        bbox_pred = t.clamp((self.decode(txtytwth_pred) / scale_t)[0], 0.0, 1.0)
